...
crearMostrar sol = do
	...
	entry2   <- xmlGetWidget xml castToEntry  "entry2"
	entry3   <- xmlGetWidget xml castToEntry  "entry3"
	entry4   <- xmlGetWidget xml castToEntry  "entry4"
	textview <- xmlGetWidget xml castToTextView "textview1"
	
	-- Añade los parámetros de la solución.
	entrySetText entry1 (show (ruta sol))
	entrySetText entry2 (show (coste_lat sol))
	entrySetText entry3 (show (coste_bw sol))
	entrySetText entry4 (show (coste_met sol))
	...
	f <- fontDescriptionNew
  	...
  	fontDescriptionSetSize f 20
  	widgetModifyFont textview (Just f)

	-- Añade el texto de la solución.
	textBufferInsertAtCursor buffer $ showE (inicial sol) ++ "\n"
	textBufferInsertAtCursor buffer $ showS (inicial sol) (lcambios sol) ++ "\n"
	textBufferInsertAtCursor buffer $ showE (final sol)
