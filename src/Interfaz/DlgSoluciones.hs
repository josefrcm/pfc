...
crearSoluciones op conf = do
	...
	-- Crea el diálogo.
	dlg_progreso <- Gtk.messageDialogNew
		Nothing
		[]
		Gtk.MessageInfo
		Gtk.ButtonsCancel
	...
	thread <- forkIO $ (do
		...
		Gtk.dialogResponse dlg_progreso Gtk.ResponseAccept
		)
...
-- 		`Control.Exception.catch` (\_ -> putMVar sols_var [])
	...
	Gtk.widgetShow dlg_progreso
	response <- Gtk.dialogRun dlg_progreso
	sols <- case response of
		Gtk.ResponseCancel -> do
			killThread thread
			return []
	...
	Gtk.widgetDestroy dlg_progreso
	return sols
